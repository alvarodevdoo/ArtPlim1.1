...
export const NotificationProvider: React.FC<NotificationProviderProps> = ({ children }) => {
  ...
  const loadNotifications = useCallback(async (page: number = 1, unreadOnly: boolean = false) => {
    try {
      setLoading(true);
      const response = await api.get('/api/production/notifications', {
  ...
  const loadUnreadCount = useCallback(async () => {
    try {
      const response = await api.get('/api/production/notifications/unread-count');
  ...
  const markAllAsRead = useCallback(async () => {
    try {
      const response = await api.post('/api/production/notifications/read-all');
