...
const shouldShowDimensions = (
  ...
  // Check valid dimensions first
  if (!item.width || !item.height) {
    return false;
  }

  // Serviços/arte não têm dimensões físicas
...
const Pedidos: React.FC = () => {
  ...
  const handleStatusChange = async (pedidoId: string, newStatus: string) => {
    try {
      ...
      // Só enviar WhatsApp automático se a automação estiver habilitada
      if (updatedPedido && updatedPedido.customer.phone && settings?.enableAutomation) {
    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {/* Valor Total - Apenas para usuários com acesso financeiro */}
          {hasFinancialAccess() && (
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                          ...
                          <div key={item.id} className="border border-border rounded-lg p-4">
                            <div className="flex justify-between items-start">
                              ...
                              <div className="text-right ml-4">
                                ...
                                {/* Botão Material - apenas para usuários com acesso financeiro */}
                                {hasFinancialAccess() && (
                                  <Button
                                    size="sm"
                                    variant="outline"
                          ...
                          {[
                            ...
                            return (
                              <div key={step.status} className="flex items-center space-x-3">
                                ...
                                {index < 3 && (
                                  <div className={`w-px h-8 ${isCompleted ? 'bg-green-200' : 'bg-gray-200'
                                    }`} />
                                )}
                              </div>
                            );
                  ...
                  {/* Profitability Analysis - apenas para usuários com acesso financeiro */}
                  {hasFinancialAccess() && (
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-lg flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <p>
                          {selectedPedido.items.reduce((total, item) =>
                            total + (((item.width || 0) * (item.height || 0) * (item.quantity || 0)) / 1000000), 0
                          ).toFixed(2)} m²
                        </p>
                      </div>
                    </CardContent>
      ...
      {/* Material Calculator Modal - apenas para usuários com acesso financeiro */}
              ...
              <CardContent>
                <MaterialCalculator
                  ...
                  onCalculationComplete={(result) => {
                    // Material calculation completed
                  }}
                />
              </CardContent>
            </Card>
