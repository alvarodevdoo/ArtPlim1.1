...
async function testPhase2Implementation() {
  ...
  try {
    ...
    // Find an existing organization first
    const organization = await prisma.organization.findFirst();
    if (!organization) {
      throw new Error('No organization found. Please run database seeding first.');
    }
    ...
    // Clean up existing configurations for this product
    const existingConfigs = await configService.listConfigurations(testProduct.id);
    for (const config of existingConfigs) {
      await configService.deleteConfiguration(config.id);
    }
    console.log(`🧹 Cleaned up ${existingConfigs.length} existing configurations`);
    ...
    const templates = await configService.listTemplates(organization.id);
