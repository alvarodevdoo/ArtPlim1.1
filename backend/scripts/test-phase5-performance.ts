...
async function testPhase5Performance() {
  ...
  try {
    ...
    const performanceSummary: any = await performanceMonitor.getPerformanceSummary();
    ...
    // Fechar conexões Redis se existirem
    try {
      const cacheService = new CacheService();
      if (cacheService.getStats().redisConnected) {
        // Aguardar um pouco para finalizar operações pendentes
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    } catch (error) {
      console.log('⚠️ Erro ao fechar conexões Redis:', error);
    }
    
    console.log('🔌 Conexões fechadas. Script finalizado.');
    process.exit(0);
  }
}
