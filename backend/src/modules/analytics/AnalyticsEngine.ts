...
export class AnalyticsEngine {
  ...
  private async getSalesMetrics(
    ...
    return result.map(row => ({
      ...
      deliveredRevenue: Number(row.deliveredRevenue)
    }));
  }
  ...
  private async getMaterialMetrics(
    ...
    return result.map(row => ({
      ...
      theoreticalConsumption: Number(row.theoreticalConsumption),
      estimatedConsumption: Number(row.estimatedConsumption),
      avgWastePercentage: Number(row.avgWastePercentage),
