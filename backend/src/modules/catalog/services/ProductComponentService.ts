...
export class ProductComponentService {
  ...
  async addComponent(productId: string, request: CreateComponentRequest) {
    ...
    const component = await this.prisma.productComponent.create({
      data: {
        ...
        wasteUnits: request.wasteUnits || 0.0,
        manualWasteUnits: request.manualWasteUnits,
  ...
  async listComponents(productId: string) {
    ...
    // Calcular estoque total para cada material
    return components.map((component: any) => ({
      ...component,
      material: {
        ...component.material,
        currentStock: component.material?.inventoryItems?.reduce((total: number, item: any) => total + item.quantity, 0) || 0
      }
    }));
  }
  ...
  async getMaterialUsageStats(materialId: string) {
    ...
    return {
      ...
      products: stats.map((component: any) => ({
