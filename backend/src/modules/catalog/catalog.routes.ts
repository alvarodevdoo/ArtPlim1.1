...
export async function catalogRoutes(fastify: FastifyInstance) {
  ...
  // Validar integridade de uma configuração
  fastify.get('/configurations/:configId/integrity', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { configId } = request.params as { configId: string };
    const prisma = getTenantClient(request.user!.organizationId);
  ...
  // Reordenar opções de uma configuração
  fastify.put('/configurations/:configId/options/reorder', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { configId } = request.params as { configId: string };
  ...
  // Duplicar configuração para outro produto
  fastify.post('/configurations/:configId/duplicate', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { configId } = request.params as { configId: string };
  ...
  // Criar template a partir de configurações de produto
  fastify.post('/products/:id/templates', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  // Aplicar template a um produto
  fastify.post('/products/:id/templates/:templateId/apply', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
  ...
  // Exportar configurações de um produto
  fastify.get('/products/:id/configurations/export', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const prisma = getTenantClient(request.user!.organizationId);
  ...
  // Importar configurações para um produto
  fastify.post('/products/:id/configurations/import', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  // Validação completa com ConfigurationValidationService
  fastify.post('/products/:id/configurations/validate-advanced', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const { selectedConfigurations } = request.body as { selectedConfigurations: Record<string, any> };
  ...
  // Detectar conflitos entre configurações
  fastify.post('/products/:id/configurations/conflicts', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const { selectedConfigurations } = request.body as { selectedConfigurations: Record<string, any> };
  ...
  // Sugerir resoluções para conflitos
  fastify.post('/configurations/conflicts/resolve', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
  ...
  fastify.post('/products/:id/configurations/validate-materials', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const { selectedConfigurations } = request.body as { selectedConfigurations: Record<string, any> };
  ...
  // Validar restrições de precificação
  fastify.post('/products/:id/configurations/validate-pricing', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const { selectedConfigurations } = request.body as { selectedConfigurations: Record<string, any> };
  ...
  fastify.post('/products/:id/configurations/validate-production', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const { selectedConfigurations } = request.body as { selectedConfigurations: Record<string, any> };
