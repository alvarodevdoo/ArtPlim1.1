  ...
  const handleAddComponent = async (materialId: string, config: any) => {
    try {
      await api.post(`/api/catalog/products/${productId}/components`, {
        ...
        wasteUnits: config.wasteUnits || 0,
        manualWasteUnits: config.manualWasteUnits,
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p>
                        {component.manualWasteUnits || component.wasteUnits > 0 
                          ? `${component.manualWasteUnits || component.wasteUnits} ${component.material.unit}`
                          : `${(component.wastePercentage * 100).toFixed(1)}%`
                        }
                      </p>
                    </div>
                    <div>
...
const EditComponentModal: React.FC<EditComponentModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    manualWasteUnits: component.manualWasteUnits?.toString() || '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updates = {
      ...
      manualWasteUnits: formData.manualWasteUnits ? parseFloat(formData.manualWasteUnits) : null,
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">
                Perdas em Unidades ({component.material.unit})
              </label>
              <input
                type="number"
                step="0.1"
                min="0"
                value={formData.manualWasteUnits}
                onChange={(e) => setFormData(prev => ({ ...prev, manualWasteUnits: e.target.value }))}
                className="w-full h-10 px-3 py-2 border border-input rounded-md bg-background"
                placeholder={`Ex: 2 ${component.material.unit}`}
              />
              <p className="text-xs text-muted-foreground">
                Atual configurado: {component.wasteUnits} {component.material.unit}
                {component.manualWasteUnits && ` (manual: ${component.manualWasteUnits} ${component.material.unit})`}
              </p>
