...
export const MaterialCalculator: React.FC<MaterialCalculatorProps> = ({
  ...
  const loadProductComponents = async () => {
    ...
    try {
      ...
      console.log('📦 Componentes carregados:', response.data.data);
      
      if (response.data.data && response.data.data.length > 0) {
        setComponents(response.data.data);
        console.log('✅ Usando componentes reais do produto');
      } else {
        console.log('⚠️ Produto não tem componentes configurados, usando dados mockados');
        loadMockMaterials();
      }
    } catch (error) {
  ...
  const loadMockMaterials = () => {
    console.log('🎭 Carregando dados mockados (fallback)');
        ...
        {components.some(c => c.id.startsWith('mock-')) && (
          <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-start space-x-2">
              ...
              <div>
                <h4 className="font-medium text-yellow-800">Dados de Demonstração</h4>
                <p className="text-sm text-yellow-700 mt-1">
                  Este produto não possui materiais configurados. Os dados exibidos são apenas para demonstração.
                  Para configurar materiais reais, acesse a página de Produtos → Configurar Materiais.
                </p>
              </div>
            </div>
          </div>
