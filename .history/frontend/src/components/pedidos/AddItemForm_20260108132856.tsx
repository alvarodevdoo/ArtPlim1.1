...
const AddItemForm: React.FC<AddItemFormProps> = ({
  ...
  const simularPreco = async () => {
    ...
    try {
      const response = await api.post('/api/sales/simulate', {
        ...
        // Para produtos por unidade, usar dimensões padrão 1x1
        width: produtoSelecionado.pricingMode === 'SIMPLE_UNIT' ? 1 : width,
        height: produtoSelecionado.pricingMode === 'SIMPLE_UNIT' ? 1 : height,
        quantity,
  ...
  const limparFormulario = () => {
    ...
    // Limpar configurações dinâmicas
  ...
  const content = (
    <div className="space-y-4">
      ...
      {produtoSelecionado && quantity > 0 && (
        produtoSelecionado.pricingMode === 'SIMPLE_UNIT' || (width > 0 && height > 0)
      ) && (
        <MaterialCalculator
          productId={produtoSelecionado.id}
          width={produtoSelecionado.pricingMode === 'SIMPLE_UNIT' ? 1 : width}
      ...
      {/* Configuration Selector - Show dynamic configurations for DYNAMIC_ENGINEER products */}
