...
export class ProductConfigurationService {
  ...
  async reorderOptions(configurationId: string, optionIds: string[]): Promise<void> {
    ...
    const existingOptionIds = configuration.options.map(opt => opt.id);
  ...
  async createTemplate(productId: string, templateName: string, userId: string): Promise<ConfigurationTemplate> {
    ...
    const template: ConfigurationTemplate = {
      organizationId: productConfigs.product.organizationId || '', // Assumindo que existe
      name: templateName,
    ...
    // Salvar template no banco (assumindo que existe uma tabela configuration_templates)
