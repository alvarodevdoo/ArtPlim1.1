...
export async function adminRoutes(fastify: FastifyInstance) {
  ...
  // Novas rotas de performance e monitoramento
  fastify.get('/performance', async (request, reply) => {
    return adminController.getPerformanceMetrics(request, reply);
  });

  fastify.get('/cache/stats', async (request, reply) => {
    return adminController.getCacheStats(request, reply);
  });

  fastify.delete('/cache', async (request, reply) => {
    return adminController.clearCache(request, reply);
  });

  fastify.get('/health', async (request, reply) => {
    return adminController.getSystemHealth(request, reply);
  });
}