...
async function testPhase5Performance() {
  ...
  try {
    ...
    console.log(`- Total de requisições: ${performanceSummary.totalRequests}`);
    console.log(`- Tempo médio de resposta: ${Math.round(performanceSummary.averageResponseTime)}ms`);
    console.log(`- Requisições lentas: ${performanceSummary.slowRequests}`);
    console.log(`- Requisições com erro: ${performanceSummary.errorRequests}`);

    if (performanceSummary.topEndpoints.length > 0) {
      console.log('📊 Top endpoints:');
      performanceSummary.topEndpoints.slice(0, 3).forEach((endpoint, index) => {
